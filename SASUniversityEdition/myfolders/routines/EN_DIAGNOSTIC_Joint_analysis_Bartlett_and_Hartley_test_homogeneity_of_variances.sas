...
title "Results of " color=RED "Bartlett " color=darkblue "test to check homogeneity of variances in Multi-Environment Trials (MET)";
data ANOVA_OUTPUT; set ANOVA_OUTPUT;
P_VALUE=(1-probchi(BARTLETT_VALUE,(NUMBER_ENV-1)));
label P_VALUE="P=Pr[X" "^{unicode 2265}" "^{unicode 03C7}²]"; 
format P_VALUE pvalue6.4; 
if P_VALUE ge 0.05 then RESULT=":) Your variances are HOMOGENEOUS - Don't Worry!";
else RESULT= ":( Your variances are HETEROGENEOUS, Be carefull";
run;
...
compute RESULT;
if  RESULT=":( Your variances are HETEROGENEOUS, Be carefull" then call define(_col_,"style","style={background=BlueViolet}");
endcomp;
run;
...
/*[4] Showing results*/
/*Creating an alert to HETEROGENEOUS variances!*/
title "Results of " color=RED "Hartley " color=darkblue "test to check homogeneity of variances in Multi-Environment Trials (MET)";
